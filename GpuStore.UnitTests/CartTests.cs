...
namespace GpuStore.UnitTests
{
    ...
    public class CartTests
    {
        ...
        {
            // Организация - создание нескольких тестовых карт
            Card card1 = new Card { CardId = 1, Name = "Карта1" };
            Card card2 = new Card { CardId = 2, Name = "Карта2" };
            // Организация - создание корзины
            Cart cart = new Cart();
            // Действие
            cart.AddItem(card1, 1);
            cart.AddItem(card2, 1);
            List<CartLine> results = cart.Lines.ToList();
            // Утверждение
            Assert.AreEqual(results.Count(), 2);
            Assert.AreEqual(results[0].Card, card1);
            Assert.AreEqual(results[1].Card, card2);
        }
        [TestMethod]
        public void Can_Add_Quantity_For_Existing_Lines()
        {
            ...
            cart.AddItem(card1, 1);
            cart.AddItem(card2, 1);
            cart.AddItem(card1, 5);
            List<CartLine> results = cart.Lines.OrderBy(c => c.Card.CardId).ToList();
            // Утверждение
            Assert.AreEqual(results.Count(), 2);
        ...
        public void Can_Remove_Line()
        {
            ...
            Card card1 = new Card { CardId = 1, Name = "Карта1" };
            Card card2 = new Card { CardId = 2, Name = "Карта2" };
            Card card3 = new Card { CardId = 3, Name = "Карта3" };
            // Организация - создание корзины
            Cart cart = new Cart();
            // Организация - добавление нескольких игр в корзину
            cart.AddItem(card1, 1);
            cart.AddItem(card2, 4);
            cart.AddItem(card3, 2);
            cart.AddItem(card2, 1);
            // Действие
            cart.RemoveLine(card2);
            // Утверждение
            Assert.AreEqual(cart.Lines.Where(c => c.Card == card2).Count(), 0);
        ...
        {
            // Организация - создание нескольких тестовых карт
            Card card1 = new Card { CardId = 1, Name = "Карта1", Price = 100 };
            Card card2 = new Card { CardId = 2, Name = "Карта2", Price = 55 };
            // Организация - создание корзины
            Cart cart = new Cart();
            // Действие
            cart.AddItem(card1, 1);
        ...
        public void Can_Add_To_Cart()
        {
            ...
            {
                new Card {CardId=1, Name="Карта1", Manufacturer="Man1"},
            }.AsQueryable());
            // Организация - создание корзины
            Cart cart = new Cart();
            ...
            Assert.AreEqual(cart.Lines.ToList()[0].Card.CardId, 1);
        }
        /// <summary>
        /// После добавления игры в корзину, должно быть перенаправление на страницу корзины
        /// </summary>
        [TestMethod]
        public void Adding_Gpu_To_Cart_Goes_To_Cart_Screen()
        {
            // Организация - создание имитированного хранилища
            Mock<ICardRepository> mock = new Mock<ICardRepository>();
            mock.Setup(m => m.Cards).Returns(new List<Card>
        ...
        public void Can_View_Cart_Contents()
        {
            ...
            CartIndexViewVodel result = (CartIndexViewVodel)target.Index(cart, "myUrl").ViewData.Model;
            // Утверждение
